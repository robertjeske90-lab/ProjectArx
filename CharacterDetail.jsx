...
import CharacterPDFExport from '../components/CharacterPDFExport';
...
export default function CharacterDetail() {
  ...
  const { getCharacter, deleteCharacter, spendExperience, updateCharacter } = useCharacters();
  ...
  // Update Cosmetics/Avatar - FIXED: Keine undefined Werte senden
  const handleCosmeticsUpdate = async (updates) => {
    try {
      ...
      await updateCharacter(id, cleanUpdates);
      const updated = await getCharacter(id);
      setCharacter(updated);
    } catch (err) {
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg font-medium transition-all bg-arx-dark text-gray-400 hover:text-arx-gold hover:bg-arx-gold/10 ml-auto"
        >
          📄 PDF Export
        </button>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Cosmetics & Avatar Editor */}
          <CosmeticsEditor 
            character={character}
            onUpdate={handleCosmeticsUpdate}
          ...
          <div className="bg-arx-dark border border-arx-purple/30 rounded-xl p-4">
            ...
            <div className="space-y-3">
              {Object.entries(gameData.vitals).map(([key, vital]) => {
                const max = character.vitals?.[key] ?? vital.baseValue;
                const current = max; // Aktuell = Max (voll)
